    ...
    def get_plugin_client_settings(self):
        if self.plugin_client == self.PluginClients.SUBLIME_TEXT_2:
            if self.platform == 'darwin':
                ...
                default_settings = mm_util.parse_json_from_file(os.path.expanduser('~/.config/sublime-text-2/Packages/MavensMate/mavensmate.sublime-settings'))
                user_settings    = mm_util.parse_json_from_file(os.path.expanduser('~/.config/sublime-text-2/Packages/User/mavensmate.sublime-settings'))
                return {
                    'user'    : user_settings,
                    'default' : default_settings
                }
            ...
            if self.platform == 'darwin':
                ...
                default_settings = mm_util.parse_json_from_file(os.path.expanduser('~/.config/sublime-text-2/Packages/MavensMate/mavensmate.sublime-settings'))
                user_settings    = mm_util.parse_json_from_file(os.path.expanduser('~/.config/sublime-text-2/Packages/Packages/User/mavensmate.sublime-settings'))
                return {
                    'user'    : user_settings,
                    'default' : default_settings
                }
